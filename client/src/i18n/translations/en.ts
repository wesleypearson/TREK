const en: Record<string, string | { name: string; category: string }[]> = {
  ...
  'admin.keyDiag.serviceBlocked': 'This key\'s API restrictions don\'t include the Places API (New) — the only Google API TREK uses. In Google Cloud Console → APIs & Services → Credentials → your key → API restrictions, add "Places API (New)", save, and test again after a few minutes.',
  'admin.keyDiag.serviceDisabled': 'The Places API (New) is not enabled on your Google Cloud project. Enable it in Google Cloud Console → APIs & Services → Library → "Places API (New)", then test again.',
  'admin.keyDiag.referrerBlocked': 'This key only allows specific HTTP referrers. TREK calls Google from the server, so set the APP_URL environment variable to an allowed referrer, or switch the key to IP-address restrictions.',
  'admin.keyDiag.ipBlocked': 'This key only allows specific IP addresses, and this server\'s IP isn\'t on the list. Add the server\'s public IP to the key\'s allowed IPs in Google Cloud Console.',
  'admin.keyDiag.badKey': 'Google rejected the key as invalid or expired. Re-copy it from Google Cloud Console → APIs & Services → Credentials.',
  'admin.keyDiag.billingDisabled': 'The Google Cloud project has no active billing account. The Places API (New) requires billing to be enabled.',

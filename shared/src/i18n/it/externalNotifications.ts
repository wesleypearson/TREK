...
const it: NotificationLocale = {
  email: {
    footer: 'Hai ricevuto questa email perché hai le notifiche abilitate in Travla.',
    manage: 'Gestisci le preferenze nelle impostazioni',
    openTrek: 'Apri Travla',
  },
  events: {
    trip_invite: (p) => ({
    ...
    schedule_change: (p) => ({
      title: `Orari modificati: ${p.trip}`,
      body: `${p.count} modifiche di orario in "${p.trip}":\n${p.summary}`,
    }),
    trip_reminder: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri Travla per accettare o rifiutare.`,
    }),
    collection_invite: (p) => ({
      title: 'Invito a una raccolta',
      body: `${p.actor} ti ha invitato a condividere una raccolta. Apri Travla per accettare o rifiutare.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nuova versione Travla disponibile',
      body: `Travla ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ciao',
    body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account Travla. Clicca il pulsante qui sotto per impostare una nuova password.',

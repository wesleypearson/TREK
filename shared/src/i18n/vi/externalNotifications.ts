...
const en: NotificationLocale = {
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} đã mời bạn kết hợp kế hoạch kỳ nghỉ. Mở Travla để chấp nhận hoặc từ chối.`,
    }),
    collection_invite: (p) => ({
      title: 'Lời mời bộ sưu tập',
      body: `${p.actor} đã mời bạn chia sẻ một bộ sưu tập. Mở Travla để chấp nhận hoặc từ chối.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Đã có phiên bản Travla mới',
      body: `Travla ${p.version} có bản mới. Vui lòng truy cập bảng điều khiển quản trị để cập nhật.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'CHÀO',
    body: 'Chúng tôi đã nhận được yêu cầu đặt lại mật khẩu cho tài khoản Travla của bạn. Nhấp vào nút bên dưới để đặt mật khẩu mới.',

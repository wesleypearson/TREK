...
const ja: NotificationLocale = {
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor}が休暇プランの統合に招待しています。Travlaを開いて承認または拒否してください。`,
    }),
    collection_invite: (p) => ({
      title: 'コレクション招待',
      body: `${p.actor}がコレクションの共有に招待しています。Travlaを開いて承認または拒否してください。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新しいTravlaバージョンが利用可能',
      body: `Travla ${p.version}が利用可能になりました。管理パネルからアップデートしてください。`,
    }),
  ...
  passwordReset: {
    subject: 'パスワードをリセット',
    greeting: 'こんにちは',
    body: 'Travlaアカウントのパスワードリセットリクエストを受け付けました。以下のボタンをクリックして新しいパスワードを設定してください。',
    ctaIntro: 'パスワードをリセット',

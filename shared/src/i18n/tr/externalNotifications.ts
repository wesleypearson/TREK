...
const tr: NotificationLocale = {
  ...
  events: {
    ...
    schedule_change: (p) => ({
      title: `Saatler değişti: ${p.trip}`,
      body: `"${p.trip}" içinde ${p.count} saat değişikliği:\n${p.summary}`,
    }),
    trip_reminder: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} sizi tatil planlarını birleştirmeye davet etti. Kabul etmek veya reddetmek için Travla'yı açın.`,
    }),
    collection_invite: (p) => ({
      title: 'Koleksiyon daveti',
      body: `${p.actor} sizi bir koleksiyonu paylaşmaya davet etti. Kabul etmek veya reddetmek için Travla’yı açın.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Yeni Travla sürümü mevcut',
      body: `Travla ${p.version} artık mevcut. Güncellemek için yönetici panelini ziyaret edin.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Merhaba',
    body: 'Travla hesabınızın şifresini sıfırlamak için bir istek aldık. Yeni bir şifre belirlemek için aşağıdaki butona tıklayın.',
